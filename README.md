...
This tunes on the locked historical seasons, scores the later holdout exactly
once, promotes a release-grade artifact, and renders every public claim from
one manifest. `src.portfolio` remains only as a compatibility alias and cannot
change the locked cohort.
...
By default, inference loads the promoted release-grade artifact from
`models/registry.json`, loads the previous and active NHL seasons, and refreshes
the active-season cache every six hours. `NHL_MODEL_PATH` is a migration-only
override and is still required to pass release-grade validation. Use
`NHL_REGISTRY_PATH` and `NHL_HISTORICAL_SEASONS` for normal configuration.
...
The exact winning configuration is stored in the tracked release manifest;
README parameters are never maintained by hand.
